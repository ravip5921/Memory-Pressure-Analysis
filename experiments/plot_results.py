...
def load_inputs(raw_path, agg_path, legacy_path):
	...
	return raw_stressed.sort_values(["Stress", "Swappiness", "Repeat"]), agg.sort_values(["Stress", "Swappiness"]), legacy, baseline_rps, baseline_p95, baseline_p99
...
def plot_slowdown_vs_swappiness(agg, outdir):
	...
	plt.title("Slowdown vs Swappiness by Stress Level")
	plt.legend(title="Stress")
	plt.tight_layout()
	plt.savefig(outdir / "slowdown_vs_swappiness.png", dpi=200)
	plt.close()
...
def plot_throughput_vs_swappiness(agg, baseline_rps, outdir):
	...
	plt.xlabel("Swappiness")
	plt.ylabel("Req/sec")
	plt.title("Throughput vs Swappiness by Stress Level")
	plt.legend(title="Stress")
	plt.tight_layout()
	plt.savefig(outdir / "throughput_vs_swappiness.png", dpi=200)
	plt.close()
...
def plot_tail_latency(agg, baseline_p95, baseline_p99, outdir):
	...
	plt.suptitle("Tail Latency Behavior")
	plt.tight_layout()
	plt.savefig(outdir / "tail_latency_vs_swappiness.png", dpi=200)
	plt.close()
...
def plot_raw_distribution(raw, outdir):
	...
	plt.axhline(1.0, linestyle="--", linewidth=1, color="gray")
	plt.title("Run-to-Run Slowdown Distribution")
	plt.ylabel("Slowdown")
	plt.xlabel("Stress")
	plt.tight_layout()
	plt.savefig(outdir / "slowdown_distribution_raw.png", dpi=200)
	plt.close()
...
def plot_heatmaps(agg, outdir):
	...
	axes[0].set_title("Mean Slowdown Heatmap")
	axes[1].set_title("Mean p99 Latency Heatmap (ms)")
	axes[0].set_xlabel("Swappiness")
	axes[1].set_xlabel("Swappiness")
	axes[0].set_ylabel("Stress")
	axes[1].set_ylabel("Stress")
	plt.tight_layout()
	plt.savefig(outdir / "heatmaps_slowdown_p99.png", dpi=220)
	plt.close()


def main():
	parser = argparse.ArgumentParser(description="Generate plots from Docker experiment summaries.")
	parser.add_argument("--raw", default="experiments/summary_runs_raw.csv")
	parser.add_argument("--agg", default="experiments/summary_aggregated.csv")
	parser.add_argument("--legacy", default="experiments/corr_rt_30_timeout_600_with_slowdown_global_baseline_linux.csv")
